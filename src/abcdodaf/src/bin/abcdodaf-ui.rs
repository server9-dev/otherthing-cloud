//! ABCDODAF BPMN IDE - Visual Workflow Editor
//!
//! Full-featured BPMN 2.0 visual editor with workspace management, validation,
//! file browser, and DoDAF 2.02 metadata support.
//!
//! # Installation
//!
//! ```bash
...
fn main() -> Result<(), eframe::Error> {
    use abcdodaf::ui::{
        ...
        PropertyEditor, TabAction, TabBar, Validator, Workspace,
    };
    use eframe::App;
    ...
    impl WorkspaceEditorApp {
        ...
        fn render_rename_dialog(&mut self, ctx: &egui::Context) -> bool {
            ...
            if let Some(dialog) = &mut self.rename_dialog {
                egui::Window::new("Rename File")
                    ...
                    .show(ctx, |ui| {
                        ui.label(format!("Current name: {}", dialog.current_name));
                        ui.separator();
            ...
            if should_rename {
                if let Some(dialog) = &self.rename_dialog {
                    let new_path = dialog.path.with_file_name(&dialog.new_name);

                    match std::fs::rename(&dialog.path, &new_path) {
                        Ok(()) => {
    ...
    impl App for WorkspaceEditorApp {
        fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            if self.rename_dialog.is_some() {
                if !self.render_rename_dialog(ctx) {
    ...
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("ABCDODAF BPMN IDE - Visual Workflow Editor"),
        ..Default::default()
    };
...
fn main() {
    ...
    eprintln!();
    eprintln!("The abcdodaf-ui binary requires the 'ui' feature to be enabled.");
    eprintln!();
    eprintln!("To install:");
    eprintln!("  cargo install --path . --features ui --bin abcdodaf-ui");
    eprintln!();
    eprintln!("To run directly:");
    eprintln!("  cargo run --bin abcdodaf-ui --features ui");
    std::process::exit(1);
}
